...
for (let c = 0; c < cols; c++) {
    for (let r = 0; r < rows; r++) {
        ...
        Object.assign(el, { tabIndex: 0, role: 'button' });
...
function updateLightbox() {
    Object.assign(lbImg, { src: imageSources[currentLbIndex].src, alt: imageSources[currentLbIndex].name });
...
burgerBtn.addEventListener('click', () => {
    ...
    if (isActive) (lastFocusedElement = document.activeElement, navOverlay.focus());
...
navOverlay.addEventListener('click', (e) => e.target === navOverlay && (burgerBtn.classList.remove('active'), navOverlay.classList.remove('active'), document.body.style.overflow = '', lastFocusedElement?.focus()));
document.getElementById('services').addEventListener('click', (e) => e.target === document.getElementById('services') && (document.getElementById('services').classList.remove('active'), lastFocusedElement?.focus()));

window.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        if (lightbox.classList.contains('active')) closeLightbox();
        if (navOverlay.classList.contains('active')) (burgerBtn.classList.remove('active'), navOverlay.classList.remove('active'), document.body.style.overflow = '', lastFocusedElement?.focus());
        if (document.getElementById('services').classList.contains('active')) (document.getElementById('services').classList.remove('active'), lastFocusedElement?.focus());
    }
